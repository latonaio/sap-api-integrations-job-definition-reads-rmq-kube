...
import (
	sap_api_caller "sap-api-integrations-job-definition-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-job-definition-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-job-definition-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	jobID := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetJobDefinition(jobID, accepter)
	return nil
}

func extractData(data map[string]interface{}) (jobID string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	jobID = sdc.JobDefinitionCollection.JobID
	return
}
