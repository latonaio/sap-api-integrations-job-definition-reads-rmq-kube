...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-job-definition-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) JobDefinitionCollection(jobID string) {
	jobDefinitionCollectionData, err := c.callJobDefinitionSrvAPIRequirementJobDefinitionCollection("JobDefinitionCollectionData", jobID)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": jobDefinitionCollectionData, "function": "JobDefinitionCollectionData"})
	if err != nil {
		c.log.Error(err)
		return
	}
	c.log.Info(jobDefinitionCollectionData)
}
